...
- [x] **Task 15.5 — Fifth Reading Mode: Page** (Implementation complete · device testing pending)
  - ✅ New engine `www/js/engines/page.js` — IIFE, same contract as RSVP/Chunk/Focus/Scroll. Same word array, same `pageWordIndex[]`; engine switches preserve word-level position both ways.
  - ✅ Registered as `page: PageEngine` in `reader.js:_engineMap`; new tab after Scroll. Narrow-phone media query (≤360px) shrinks tab font to fit five tabs on one row.
  - ✅ DOM-measured greedy fill pagination chunked at 250 words/rAF with `_updateEngineLoadingProgress()`. Measurement page is auto-height, off-screen (`left: -10000px; width: shell.clientWidth`) so `offsetHeight` reflects content size — the `inset: 0` original implementation produced one-word pages because `scrollHeight` was always >= `clientHeight`. Fallback `_paginateByWordCount()` (180 wpp) handles unlaid-out shells. Cache keyed by `${fileId}|${fontPreset}|${fontScale}|${vw}x${vh}` — auto-invalidates on font/scale/orientation change.
  - ✅ Three-slot sliding window (prev/current/next at `translateX(-100%/0/100%)`) with `0.25s cubic-bezier` snap via `.page-shell-animating` class. Swipe (25% width OR 0.5 px/ms velocity), edge damping at corpus boundaries, vertical-cancel threshold so system back-edge swipe still works.
  - ✅ Bottom `.page-engine-nav` bar — `[⏮ ‹ "x/y" › ⏭ PDF]` where the trailing bridge button is the PDF / URL / IMG proxy picked by `_findBridgeButton()`. Buttons disable at boundaries. `body.engine-page` toggled by `_applyEngineChrome` hides standard `#wpm-bar` + `#playback-bar` AND the floating `.reader-normal-toggle` (which overlapped reflowed text in non-calm Page mode).
  - ✅ `.reader-header` + `.engine-tabs` got `z-index: 11; background: var(--bg)` so they never get visually obscured by the engine-loading card during a build — mode tabs are tappable throughout pagination.
  - ✅ Tap-to-toggle chrome: **calm-mode only**. When calm is ON, tap toggles `.page-chrome-hidden` and auto-hides after 4 s. When calm is OFF, taps do nothing and chrome stays visible. Turning calm off clears `page-chrome-hidden` via `_applyCalmMode()` so chrome can't get stuck hidden.
  - ✅ Inherits existing systems: `WordTapFeature.bindWord` (long-press dictionary), `openObjectPlaceholder` (placeholder → Normal view bridge), `acquireWakeLock` on init + `startIdleReleaseTimer` on destroy. Page mode has no play button — `init` directly calls `_onEnginePlay()` so streak/heatmap/Task 15.4 notification reschedule all credit Page-mode reading.
  - ✅ New CSS var `--page-stage-bg: #161812`. Inline `--font-scale` propagates the settings slider into the leaf font size. Calm mode dims the nav bar at 15%.
  - ✅ Engine name "Page" stays Roman in both EN and HI per §15.6. Two new loading i18n keys.
  - ✅ versionCode 27 / versionName "1.4".
  - ⏳ Device testing pending — pagination across short/long books, swipe feel, position preservation across engine switches, placeholder round-trip, font-scale and orientation re-pagination, wake lock, session credit.
